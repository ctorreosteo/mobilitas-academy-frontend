...
import {
  creaPrenotazioneSessionePosturale,
  fetchCalendarioSessioniPosturali,
  fetchPartecipazioniSessioniPosturali,
  fetchSessioniPosturali,
  parseSessionePosturaleTags,
  type CalendarioSessionePosturaleDto,
  type SessionePosturaleDto,
} from '../../services/sessioniPosturaliService';
import {
  applySessioniPosturaliCatalog,
  getSessioniPosturaliCatalog,
  getSessioniPosturaliCoversRecord,
...
const SessioniCalendarioScreen: React.FC = () => {
  const tabBarPad = useTabBarBottomPadding();
  const todayIso = useMemo(() => toIsoDate(new Date()), []);
  const [selectedDate, setSelectedDate] = useState(todayIso);
  const [visibleMonth, setVisibleMonth] = useState(startOfMonth(new Date()));
  const [calendarRows, setCalendarRows] = useState<CalendarioSessionePosturaleDto[]>([]);
  const [coverBySessionId, setCoverBySessionId] = useState<Record<number, string | null>>({});
  const [catalogById, setCatalogById] = useState<Record<number, SessionePosturaleDto>>({});
  ...
  const [confirmSession, setConfirmSession] = useState<CalendarioSessionePosturaleDto | null>(null);
  const [bookedSessionName, setBookedSessionName] = useState<string | null>(null);
  const [error, setError] = useState<string | null>(null);

  const syncCatalogState = useCallback((sessioni: SessionePosturaleDto[]) => {
    applySessioniPosturaliCatalog(sessioni);
    setCoverBySessionId(Object.fromEntries(sessioni.map((s) => [s.id, s.immagineCopertinaUrl ?? null])));
    setCatalogById(Object.fromEntries(sessioni.map((s) => [s.id, s])));
  }, []);

  useEffect(() => {
    setCoverBySessionId(getSessioniPosturaliCoversRecord());
    setCatalogById(Object.fromEntries(getSessioniPosturaliCatalog().map((s) => [s.id, s])));
    return subscribeSessioniPosturaliCatalog(() => {
      setCoverBySessionId(getSessioniPosturaliCoversRecord());
      setCatalogById(Object.fromEntries(getSessioniPosturaliCatalog().map((s) => [s.id, s])));
    });
  }, []);
  ...
  const loadCalendar = useCallback(async () => {
    ...
    try {
      const [calendar, bookings, sessioni] = await Promise.all([
        fetchCalendarioSessioniPosturali({ data: selectedDate }),
        fetchPartecipazioniSessioniPosturali(),
        fetchSessioniPosturali().catch(() => []),
      ]);
      if (sessioni.length > 0) {
        syncCatalogState(sessioni);
      }
      setCalendarRows(calendar);
      setMySessionIds(new Set(bookings.map((item) => item.sessioneId)));
    } catch (e) {
      setError(
        getUserFacingApiErrorMessage(e, {
          context: 'Impossibile caricare il calendario sessioni',
          fallback:
  ...
  }, [selectedDate, syncCatalogState]);

  useFocusEffect(
    useCallback(() => {
      void prefetchSessioniPosturaliCatalog();
      loadCalendar();
    }, [loadCalendar])
  );

  const getSessionCoverUrl = useCallback(
    (row: CalendarioSessionePosturaleDto) =>
  ...
  const onConfirmBook = useCallback(
    async (row: CalendarioSessionePosturaleDto) => {
      setBookingSessionId(row.sessioneId);
      try {
        await creaPrenotazioneSessionePosturale({ sessioneId: row.sessioneId });
        await loadCalendar();
      ...
      <ScrollView contentContainerStyle={[styles.scrollContent, { paddingBottom: 30 + tabBarPad }]}>
        ...
        {!loading && error ? (
          <View style={styles.stateCard}>
            ...
            <StudioWhatsAppSupportButton
              prefilledMessage={SESSIONI_CALENDAR_WHATSAPP}
        ...
        {!loading && !error
          ? calendarRows.map((row) => {
              ...
              const catalog = catalogById[row.sessioneId];
              const tags = parseSessionePosturaleTags(catalog?.tags);
              return (
                <View key={row.id} style={styles.sessionCard}>
                  ...
                  {row.istruttoreNomeCompleto ? (
                    <Text style={styles.sessionMeta}>Istruttore: {row.istruttoreNomeCompleto}</Text>
                  ) : null}
                  {catalog?.durata ? (
                    <Text style={styles.sessionMeta}>Durata: {catalog.durata} min</Text>
                  ) : null}
                  {tags.length > 0 ? (
        ...
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalText}>
              ...
              {confirmSession ? toHour(confirmSession.oraInizio) : '--:--'}? La prenotazione vale per questo tipo
              di sessione, non solo per l’orario scelto.
            </Text>
...
export default SessioniCalendarioScreen;
